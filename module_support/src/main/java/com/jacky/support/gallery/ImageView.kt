...
import com.jacky.support.utils.ImageUtils
...
object ImageStyle {
    val CommonRequestOptions: RequestOptions.() -> Unit = {
...
private fun Any?.toImageSource(): ImageSource {
    return when {
        ...
        this is String -> {
            if (File(this).exists()) LocalImageSource(
                this
            )
...
suspend fun List<LocalImageSource>?.compress(@NotNull context: Context): List<File?>? {
    return this?.map {
        (it.get() as? String)?.let { path ->
            ImageUtils.compress(context, path)
        }
    }
}
